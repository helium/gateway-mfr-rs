...
impl Device {
    ...
    pub fn get_keypair(&self, create: bool) -> Result<Keypair> {
        ...
        let keypair = tpm::init().and_then(|_| {
            tpm::Keypair::from_key_path(Network::MainNet, self.path.as_str())
...
fn check_miner_key(key_path: &str) -> TestResult {
    let keypair = tpm::init().and_then(|_| {
        tpm::Keypair::from_key_path(Network::MainNet, key_path).map(helium_crypto::Keypair::from)
    })?;
